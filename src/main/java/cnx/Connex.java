...
public class Connex{
    ...
    public static Connection PsqlConnect(){
        ...
        {
                Class.forName("org.postgresql.Driver");
                c = DriverManager.getConnection("jdbc:postgresql://localhost:5432/meubledata","postgres","mdpprom15");
        }
        catch(Exception e){
                System.out.println("Erreur de connexion");
                e.printStackTrace();
    ...
    public static String createId(Connection cnx, String nomsequence, String prefixe, int len) throws Exception
    {
        String id = "";
        ...
        {
            int nbZero = len - (prefixe.length() + (seq + "").length());
            String complement = "";
            for(int i = 0 ; i < nbZero ; i++){
                complement += "0";
            }
            id = prefixe + complement + seq;
        }
